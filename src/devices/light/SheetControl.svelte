...
>
  ...
  >
    <div class="flex flex-row items-center p-4">
      ...
      />
      <LightInformation {switchState} {label} />
      <div
        class="text-black flex grow flex-row justify-end items-stretch relative"
      >
        <!-- svelte-ignore a11y-click-events-have-key-events -->
        <!-- svelte-ignore a11y-no-static-element-interactions -->
        <div
          class="-top-2 relative text-black w-8 h-8 rounded-full bg-gray-100 p-2 cursor-pointer"
          on:click={(e) => close()}
        >
          <svg
